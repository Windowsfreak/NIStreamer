...
public class Main {
    public static PreferenceFragment pf;
    final String name;
    public final Config config;
    public final SourceFactory sif;
    AbstractCodec codec;
    ...
    public final ArrayList<CompressionWorker> workers = new ArrayList<CompressionWorker>();
    int nextIn = 0;
    int nextOut = 0;
    int threads;
    public final Thread schedulerThread;
    public final Thread deliveryThread;
    ...
    class CompressionWorker extends Thread {
        ...
        void deliver() {
            ...
            if ((ucount & 15) == 0 && unew - ubase > 5000) {
                final String fcdS = config.frameId - frameIds[threadId] + " frame";
                final String fpsS = (ucount * 100000 / (unew - ubase)) / 100f + " fps";
                final String ratioS = (ucomp * 10000 / ubytes) / 100f + "%";
                final String bandwidthS = (ucomp / (unew - ubase)) + " thousand bytes/s";
                pf.getActivity().runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        final Preference fcd = (Preference) (pf.findPreference("fcd"));
                        fcd.setSummary(fcdS);
                        final Preference fps = (Preference) (pf.findPreference("fpsb"));
                        fps.setSummary(fpsS);
                        final Preference ratio = (Preference) (pf.findPreference("ratio"));
                        ratio.setSummary(ratioS);
                        final Preference bandwidth = (Preference) (pf.findPreference("bandwidth"));
                        bandwidth.setSummary(bandwidthS);
                    }
                });
                ubase = unew;
