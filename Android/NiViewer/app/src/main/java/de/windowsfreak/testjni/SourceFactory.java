...
import de.windowsfreak.nistreamer.Installation;
...
public abstract class SourceFactory {
    ...
    static {
        ...
        sif.add(new SourceFactory("kinect") {
            ...
            public void start(final Config config) {
                ...
                if (installation.execute()) {
                    System.out.println("ROOT commands executed.");
                } else {
                    System.out.println("Not running as ROOT.");
                }
                osr = new OniStreamSource(activity);
                osr.initialize();
                try {
                    synchronized (osr) {
                        while (!osr.isInitialized()) {
                            osr.wait();
                        }
                    }
        ...
        sif.add(new SourceFactory("oni") {
            ...
            public void start(final Config config) {
                super.start(config);
                ofr = new OniFileSource(activity);
